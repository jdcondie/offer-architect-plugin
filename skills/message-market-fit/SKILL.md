...
## Hormozi Messaging Lens

When auditing messaging, layer these Hormozi filters on top of the 5 Pillars:

### Value Equation as Messaging Diagnostic
Score messaging against all four levers:
- **Dream Outcome clarity:** Does the copy describe the destination, not the vehicle? ("Look good naked" not "gym membership")
- **Perceived Likelihood signals:** Is proof density high enough that it's unreasonable NOT to believe?
- **Time Delay communication:** Is speed/fast-win messaging present? ("The only thing that beats free is fast.")
- **Effort & Sacrifice framing:** Is the effort level clear? Done-for-you messaging commands premium positioning.

### Niche Specificity Multiplier
Generic weight loss course = $19. Weight loss for shift nurses = $1,997. Same core content.
When ICP Alignment scores low, check: is the messaging specific enough to command a premium, or is it commodity-level generic? The more specific the niche, the rarer the solution, the higher the price.

### Close Rate as Messaging Signal
If the client shares close rate data during the audit:
- 80%+ → messaging is working but they're underpriced 3-4x
- 35-40% → sweet spot
- Below 30% → messaging, targeting, or offer problem (the audit should surface which)

This helps distinguish "messaging is wrong" from "messaging is fine but the economics are broken."

### The Starving Crowd Check
Before optimizing messaging, verify the market has: massive pain, purchasing power, easy to target, and growing. If the market fails any of these, better messaging won't save a bad market choice.
